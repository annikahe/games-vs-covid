breed [essential-workers essential-worker]
breed [workers worker]

turtles-own
[
  infected?            ;;if true, the person is infectious
  symptomatic?         ;;if true, the person is symptomatic
...
to setup-people
  ...
  set-default-shape essential-workers "person doctor"
  ...
  [
    ...
    set breed workers
    set is-working? true
    set player-strategy 0
    become-susceptible
    set score-player 0
  ]

  ask n-of 10 turtles [
    set player-strategy 1
    set color orange
  ]
...
to distribute-essential-workers
  ask n-of (essential-workers-frac * (count turtles)) turtles
  [ set breed essential-workers
    set is-working? true
  ]
end

to distribute-cost-stay
  ask workers
  [
    set cost-stay random-normal 10 3
  ]
end
...
to spread-virus
  ask turtles with [infected? and is-working?]
    [ ask link-neighbors with [not recovered? and not infected? and is-working?]
...
to decide
  ...
  if (not symptomatic?) and (random 10 < 5)
    [ attend-work ]
  ]
...
;    [ attend-work ]
;  ]
;  if strategy = "risk-factor-strategy" [
;  if (not symptomatic?) and (cost-stay > risk-factor * cost-disease)
;  [ attend-work ]
;  ]
...
"" ""
PENS
"working" 1.0 0 -13345367 true "" "plot count turtles with [is-working?]"
...
strategy
strategy
"random-strategy" "risk-factor-strategy"
1
